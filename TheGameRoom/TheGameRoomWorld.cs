...
namespace TheGameRoomWorld

{

	public class TheGameRoomWorld : ModWorld
		{
    ...
    public override void ModifyWorldGenTasks(List<GenPass> tasks, ref float totalWeight)
    {
		...
		if(shiniesIndex != -1)
		{
			tasks.Insert(shiniesIndex +1, new PassLegacy("Salt Ore Generation", OreGeneration));
		}
		...
		private void OreGeneration(GenerationProgress progress) {
			...
			for (int k = 0; k < (int)((Main.maxTilesX * Main.maxTilesY) * 6E-05); k++) {
				...
				WorldGen.TileRunner(x, y, WorldGen.genRand.Next(3, 6), WorldGen.genRand.Next(2, 6), TileType<Salt>());
